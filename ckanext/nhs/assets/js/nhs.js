this.ckan.module("report-dataset", function (jQuery) {
  return {
    ...
    _onSubmit: function () {
      ...
      if (
        $("#field-type option:selected").text() ==
        "Select an issue type from the list"
      ) {
        issue_type_el.after(
          '<span class="error-block">Please select an issue type from the list</span>',
        );
        error = true;
      }

      if (description_el.val().length == 0) {
        description_el.after(
          '<span class="error-block">Please enter a description.</span>',
        );
        error = true;
      }
